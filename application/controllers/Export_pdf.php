...
class Export_pdf extends CI_controller
{
    ...
    {
        $siswa = $this->db->query("SELECT nisn,nis,nama FROM tb_siswa WHERE kode_instansi = $kodeInstansi AND kode_program = $kodeProgram")->result();
        $
        ...
        foreach ($data_kegiatan as $kegiatan) {
            ...
            $pdf->Ln(5);
            $data_rekening = $this->db->query("SELECT tb_rekening.kode_patokan,tb_rekening.kode_rekening,tb_rekening.uraian_rekening,tb_rekening.total_rinci,
                ...
                FROM `tb_rekening` 
                WHERE tb_rekening.kode_instansi = $kodeInstansi AND tb_rekening.kode_program = $kodeProgram AND tb_rekening.kode_kegiatan = $kodeKegiatan
            ...
            foreach ($data_rekening as $rekening) {
                ...
                $pdf->Cell(85,5,@$rekening->uraian_rekening,1,0);
        ...
        $pdf->SetX(220);
        $pdf->Cell(30,10,"Sidoarjo, ".date("d F Y"),0,1);
        $pdf->SetX(222);
        $pdf->Cell(40,0,"Nama Sekolah",1,1,"C");
        $pdf->Ln(20);
        $pdf->SetX(222);
        $pdf->Cell(40,0,"Nama Siswa",1,1,"C");
        $pdf->Ln(5);
        $pdf->SetX(222);
        $pdf->Cell(40,0,"Nis",1,1,"C");
